...
#with open("example.txt", "r") as archivo:
    ...
    for paso in range (0 ,1000):
        par_conexion = lista_ordenada_por_valor[paso*2]
        print()
        print("paso ",paso)
        crear_conexion(par_conexion[0])

    print(len(cajas_a_remover))
    print(len(circuitos))

    lista_ordenada = ordena_circuitos()
    max = len(lista_ordenada)

    max_1 = (lista_ordenada[max -3])
    max_2 = (lista_ordenada[max -2])
    max_3 = (lista_ordenada[max -1])
    print((max_1*max_2*max_2))
