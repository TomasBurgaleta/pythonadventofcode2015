...
def retirar_rollos(matriz):
    contador_rollos_retirados = 0
    condicion = matriz > 0
    ...
    for fila, columna in zip(indices_filas, indices_columnas):
        ...
        if (num_rollos_proximos <= max_rollos_prox):
            contador_rollos_retirados += 1
            matriz[fila, columna] = 0
    return contador_rollos_retirados
...
#with open("example.txt", "r") as archivo:
    ...
    while True:
        rollos_retirados = retirar_rollos(matriz)
        print(rollos_retirados)
        contador += rollos_retirados
        print(contador)
        if rollos_retirados == 0 :
            break
    print(contador)